...
class TestShowSummary:
    @pytest.mark.parametrize("club, competition", [(0, 0)], indirect=['club', 'competition'])
    def test_booking_past_events_should_show_error(self, client, mocker, club, competition, clubs, competitions):
        """
        ...
        mocker.patch.object(server, 'competitions', competitions)
        response = client.get(f'/book/{competition["name"]}/{club["name"]}')
        expected_value = 'You can&#39;t book events in the past'
        assert expected_value in response.data.decode('unicode_escape')
